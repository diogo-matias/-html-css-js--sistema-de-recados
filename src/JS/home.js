...
function salvar(event) {
    ...
    if (!descricao.value && !detalhamento.value) {
        alertShow("Preencha pelo menos um campo", "danger", 1000);
        return;
    }
    if (!descricao.value) {
...
function carregarHTMLTabela() {
    ...
    <tr class="linha">
      ...
      <td class="more-info-button">
        <button 
        class="btn btn-success info-btn" 
        id="${linha.id}"
        name = "info-btn"
...
function adicionarHTMLConfirmacao() {
  ...
  <div class="container-confirmacao">
    ...
    <button id="btnSimConfirmacao" class="botao botao-confirmacao bg-success"> SIM </button>
    <button id="btnNaoConfirmacao" class="botao botao-confirmacao bg-danger"> NÃO </button>
  </div>
  `;
...
>
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-dark-trasnparent">
      ...
      <div class="modal-body">
        <form id="modal-form">
          ...
          <div class="mb-3">
            <label for="recipient-details" class="col-form-label"
              >Detalhamento</label
            >
            <textarea class="form-control bg-dark text-light" id="recipient-details">${details}</textarea
            >
          </div>
        ...
        <button type="button" data-bs-dismiss="modal" class="btn btn-success" onclick="salvarRecado('${description}')">
          Salvar
        </button>
...
function moreInfo(buttonId) {
    ...
    loadModal(descriptionElement.value, detailsElement.value);
    $("#myModal").modal("show");
}
